...
/*DEBUG: fprintf(stderr, "save_word_2_file() is invoked with repeater=%d, word='%s', numOfFile=%d, numOfWordreps=%d\n", repeater, word, numOfFile, numOfWordreps);*/
  unsigned int cod = hash(word, 0), i;
  char str[100], str2[100];
  while (1)
  {
    lseek(wordfile, (LEN_LEXEM + 1 +
		     repeater * PATTERN_LENGTH +
		     PLATFORM_STRING_END + 1) * (long)cod, SEEK_SET);
    read(wordfile, str, LEN_LEXEM+1);
    str[LEN_LEXEM]=0;
...
/*DEBUG: fprintf(stderr, "save_word_2_file() returns with new word '%s'\n", str2);*/
    ...
    {
      unsigned int counter, min = 0, minindex = 0;
      for(i = 0; i < repeater; i++)
      {
        read(wordfile, str, PATTERN_LENGTH);
        str[PATTERN_LENGTH - 1] = 0; /*nuzhno otrezat' ';'*/
        counter = atoi(strchr(str, '=') + 1);
        if(!counter) /*mesto svobodno!*/
        {
          lseek(wordfile, -PATTERN_LENGTH, SEEK_CUR);
          sprintf(str, PATTERN_LOCATION, numOfFile, numOfWordreps);
          write(wordfile, str, PATTERN_LENGTH);
    /*DEBUG: fprintf(stderr, "save_word_2_file() returns after storing word\n");*/
          return;
        }
        if(counter < min || !min) /*tekushee slovo vstrechaetsya rezhe chem proshliy minimum*/
        {
          minindex = i;
          min = counter;
        }
      ...
      if (min < numOfWordreps) /*umret kto-to na pozicii*/
      {
        lseek(wordfile, (LEN_LEXEM + 1 +
            ...
            (LEN_LEXEM + 1) +
            PATTERN_LENGTH*minindex, SEEK_SET);
        sprintf(str, PATTERN_LOCATION, numOfFile, numOfWordreps);
        write(wordfile, str, PATTERN_LENGTH);
      }
