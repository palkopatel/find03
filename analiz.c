...
/*-------------------------------------*/
char lex[10 * LEN_LEXEM], tag[LEN_TAG], taginside[LEN_TAGINSIDE];
...
{
  char href[5];
  if (is_teg_w_link(str1, href))
  {
    char *ptr;
    unsigned int len = 0;
    if (!strlen(taginside))
      return 3; /*u tega net atributov*/
    while (is_separator(taginside[0]))
      strcpy(taginside, taginside + 1);
    while (1) /*na sluchay, esli pered 'href' kakoy-to musor, opirayas' na znak "=" sdelaem neskol'ko prohodov*/
    {
      if (len >= strlen(taginside)) break;
      len += str_lwr(taginside + len) + 1;
      ptr = strstr(taginside, href);
      if (ptr)
      {
	int i;
	if (strchr(ptr, '=')) ptr = strchr(ptr, '=') + 1;
	else return 1; /*<a href> s oshibkoy -- bez '=' :-?*/
	for ( ; is_mini_separator(*ptr) && *ptr; ptr++)
/*	if (*ptr == '"'){ptr++;break;}*/;
